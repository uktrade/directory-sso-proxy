...
def test_flag_user_email_with_signed_client(signed_client):
    url = reverse('user_by_email', kwargs={'email': 'some@user.com'})
...
def test_flag_user_email_with_signed_client_without_required_email(
        signed_client):
    url = reverse('user_by_email', kwargs={'email': ''})
    data = {'is_verified': True}
    response = signed_client.patch(url, data=data)
...
def test_flag_user_email_with_unsigned_client(client):
    url = reverse('user_by_email', kwargs={'email': 'some@user.com'})
    data = {'is_verified': True}
    response = client.patch(url, data=data)
    assert response.status_code == status.HTTP_403_FORBIDDEN
